...
std::mutex TRFScalerQueue::All;
std::mutex TRFScalerQueue::Sorted;
...
TRFScalerQueue* TRFScalerQueue::fRFScalerQueueClassPointer = nullptr;

TRFScalerQueue* TRFScalerQueue::Get()
{
   /// Get a pointer to the global scaler Q.
   if(fRFScalerQueueClassPointer == nullptr) {
      fRFScalerQueueClassPointer = new TRFScalerQueue;
   }
   return fRFScalerQueueClassPointer;
}

TRFScalerQueue::TRFScalerQueue()
{
   fRFScalerQueueClassPointer = this;
   fScalersInQueue              = 0;
   // When the Global Q is created, start a timer to see how long we are using it.
   fStopwatch = new TStopwatch();
   fStopwatch->Start();
...
TRFScalerQueue::~TRFScalerQueue() = default;

void TRFScalerQueue::Print(Option_t*) const
{
   /// Print the status of the Scaler Queue
   CheckStatus();
}

void TRFScalerQueue::Clear(Option_t*)
{
   /// Clear the entire Queue, Queue counters, and timer.
   bool locked = false;
   if(!fRFScalerQueue.empty()) {
      while(!TRFScalerQueue::All.try_lock()) {
         // do nothing
      }
      locked = true;
   }
   ...
   if(locked) {
      TRFScalerQueue::All.unlock();
   }
   return;
}

void TRFScalerQueue::StartStatusUpdate()
{
   /// The status thread runs the status update at various intervals to show the progress of the analysis tree.
   fStatusUpdateOn = true;

   std::thread statusUpdate(&TRFScalerQueue::StatusUpdate, this);
   statusUpdate.detach();
}

void TRFScalerQueue::StopStatusUpdate()
{
   /// Stops the status update
   fStatusUpdateOn = false;
}

void TRFScalerQueue::Add(TScalerData* scalerData)
{
   /// Add a Scaler to the scaler Queue.
   if(scalerData == nullptr) {
      return;
   ...
   while(!TRFScalerQueue::Sorted.try_lock()) {
      // do nothing
   }

   fRFScalerQueue.push(scalerData);

   fTotalScalersIn++;
   fScalersInQueue++;
   fScalersIn++;

   TRFScalerQueue::Sorted.unlock();
}

void TRFScalerQueue::Pop()
{
   // Take a scaler out of the Queue
   while(!TRFScalerQueue::Sorted.try_lock()) {
      // do nothing
...
TScalerData* TRFScalerQueue::PopScaler()
{
   // Take a scaler out of the Queue and return a pointer to it.
   while(!TRFScalerQueue::Sorted.try_lock()) {
      // do nothing
   ...
   if(Size() > 0) {
      TScalerData* scaler = fRFScalerQueue.front();
      if(scaler != nullptr) {
         fRFScalerQueue.pop();
         fScalersInQueue--;
         fScalersOut++;
...
int TRFScalerQueue::Size() const
{
   // Returns the number of scalers in the Queue
   return fScalersInQueue;
}

void TRFScalerQueue::CheckStatus() const
{
   // Checks the status of the Queue. This is called by the Print() function.
   while(!TRFScalerQueue::All.try_lock()) {
      // do nothing
...
void TRFScalerQueue::StatusUpdate()
{
   // Updates the status of the scaler Queue
   float time            = 0;
   float scaler_rate_in  = 0;
...
void TRFScalerQueue::ResetRateCounter()
{
   // Resets the number of scalers in and scalers out counter. This is useful for checking to see if the rate
   // of scaler parsing is changing.
   while(!TRFScalerQueue::All.try_lock()) {
